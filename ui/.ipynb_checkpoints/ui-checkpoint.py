...
from pipeline.scale import scale_folder_by
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="Aspect Wizard Orchestrator") as demo:
        ...
        with gr.Tab("3) Resolution Scaling"):
            ...
            # Choose scaling mode + valid Flux targets
            with gr.Row():
                mode = gr.Radio(
                    choices=["Long side", "Width", "Height"],
                    value="Long side",
                    label="Scale by"
                )

                # Flux standard options extracted from your table
                FLUX_TARGETS = [
                    1408, 1728, 1664, 1920, 2176,   # 2MP
                    1024, 1216, 1152, 1344, 1536    # 1MP
                ]
                target = gr.Dropdown(
                    label="Target pixels (Flux standard)",
                    choices=[str(v) for v in FLUX_TARGETS],
                    value="1664"
                )
            ...
            # Scaling runner
            def on_scale(folder, m, t):
                if not folder:
                    raise gr.Error("Pick a folder first.")
                try:
                    out_dir, saved = scale_folder_by(folder, m, int(t))
        ...
        with gr.Tab("7) Full Automation"):
            ...
            # Img2Img + Inpaint settings combined
            with gr.Accordion("Settings", open=False):
                with gr.Row():
                ...
                with gr.Row():
                    dd_sampler_auto   = gr.Dropdown(label="Sampler", choices=SAMPLERS, value="DPM2", interactive=True)
                    dd_scheduler_auto = gr.Dropdown(label="Schedule", choices=SCHEDULERS, value="Simple", interactive=True)
        
                with gr.Row():
                    fs_steps_auto = gr.Number(value=30, label="Steps")
                    fs_cfg_auto   = gr.Number(value=1.0, label="CFG Scale")
        
                with gr.Row():
                    fs_width_auto  = gr.Number(value=1216, label="Width (None = auto)")
                    fs_height_auto = gr.Number(value=1664, label="Height (None = auto)")
        
                with gr.Row():
                    fs_denoise_auto_i2i = gr.Number(value=0.45, label="Img2Img Denoising strength")
                    fs_denoise_auto_inp = gr.Number(value=0.45, label="Inpaint Denoising strength")
                    fs_bsize_auto   = gr.Number(value=1, label="Batch size")
                    fs_bcnt_auto    = gr.Number(value=1, label="Batch count (n_iter)")
        
                fs_seed_auto = gr.Number(value=-1, label="Seed (-1=random)")
        
                # Inpaint-only controls
                with gr.Row():
            ...
            # Main pipeline runner (per-image sequential pipeline)
            def run_full_pipeline(
                folder, model_choice, lora_choice, lora_weight,
                sampler, scheduler, stp, cfg, w, h, den_i2i, den_inp, bs, bc, sd,
                ...
                for idx, img in enumerate(images, 1):
                    ...
                    # Step 2: Masking (on first Img2Img output)
                    masks = []
                    ...
                    # Step 3: Inpainting
                    fs = ForgeSettings(**i2i_settings.__dict__)
                    fs.denoising_strength = float(den_inp)
                    fs.inpaint_full_res = (inpaint_area_choice == "Whole picture")
                    fs.inpaint_full_res_padding = int(inpaint_padding_px)
        
                    img_inp = []
                    for out_inp, saved_inp, status in run_forge_inpaint_single(img_outs[0], m_dir, fs):
            ...
            def save_template_fn(name, *vals):
                ...
                keys = [
                    "model_choice","lora_choice","lora_weight","sampler","scheduler",
                    "steps","cfg","width","height",
                    "denoise_i2i","denoise_inpaint",
                    "batch_size","batch_count","seed",
            ...
            def load_template_fn(name):
                ...
                return [
                    ...
                    gr.update(value=data.get("sampler")),
                    gr.update(value=data.get("scheduler")),
                    gr.update(value=data.get("steps")),
                    gr.update(value=data.get("cfg")),
                    gr.update(value=data.get("width")),
                    gr.update(value=data.get("height")),
                    gr.update(value=data.get("denoise_i2i", 0.45)),
                    gr.update(value=data.get("denoise_inpaint", 0.45)),
                    gr.update(value=data.get("batch_size")),
                    gr.update(value=data.get("batch_count")),
                    gr.update(value=data.get("seed")),
            ...
            btn_auto.click(
                ...
                inputs=[
                    ...
                    dd_sampler_auto, dd_scheduler_auto,
                    fs_steps_auto, fs_cfg_auto, fs_width_auto, fs_height_auto,
                    fs_denoise_auto_i2i, fs_denoise_auto_inp,
                    fs_bsize_auto, fs_bcnt_auto, fs_seed_auto,
                    st_scaled_path, fs_prompt_auto,
                    inpaint_area_auto, inpaint_padding_auto
                ],
